...
class VerificationView(discord.ui.View):
    ...
    async def verify_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not role_id:
            await interaction.response.send_message(embed=ErrorEmbed("Verification role is not set up correctly."), ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.response.send_message(embed=ErrorEmbed("I don't have permission to assign the verification role."), ephemeral=True)
...
class Security(commands.Cog):
    ...
    @app_commands.command(name="spawn_verification", description="Spawn the verification panel")
    ...
    async def spawn_verification(self, interaction: discord.Interaction):
        ...
        if not settings.get('verification_enabled') or not settings.get('verification_role_id'):
            await interaction.response.send_message(embed=ErrorEmbed("Verification is not enabled. Use `/setup`."), ephemeral=True)
            return

        embed = SyncInkEmbed(title="🛡️ SyncInk Verification")
        embed.description = "To gain access to the server, please click the **Verify** button below."
