...
def draw_circle_avatar(base: Image.Image, avatar_img: Image.Image, x: int, y: int, size: int):
...
def draw_accent_bar(draw: ImageDraw.Draw, x: int, y: int, height: int, color: Tuple[int, int, int]):
...
class StatsImageService:
    ...
    async def _fetch_image(url: str) -> Optional[Image.Image]:
        """Downloads an image from URL safely with Discord-friendly headers."""
        headers = {
            "User-Agent": "SyncInk-Bot (Discord.py, https://github.com/SyncInk)"
        }
        try:
    ...
    @staticmethod
    def _get_syncink_logo(size: int = 24) -> Optional[Image.Image]:
    ...
    @classmethod
    async def generate_server_stats_card(cls, guild: Any) -> io.BytesIO:
        """
        ...
        if getattr(guild, 'icon', None):
            try:
                icon_img = await cls._fetch_image(guild.icon.url)
                if icon_img:
        ...
        if not icon_drawn:
            # Stylish fallback initial circle
            draw.ellipse([icon_x, icon_y, icon_x + icon_size, icon_y + icon_size], fill=COLOR_ACCENT_PURPLE)
            initials = "".join([w[0].upper() for w in guild.name.split()[:2]]) or "SI"
        ...
        guild_name = f"/{guild.name.lower().replace(' ', '-')}"
        draw.text((icon_x + icon_size + 18, icon_y + 8), guild_name, font=name_font, fill=COLOR_TEXT_WHITE)
        
        member_count = getattr(guild, 'member_count', len(getattr(guild, 'members', [])))
        online_count = sum(1 for m in getattr(guild, 'members', []) if getattr(m, 'status', None) and m.status.name in ('online', 'idle', 'dnd'))
        ...
        created_str = created_at_dt.strftime("%b %d, %Y")
        ...
        channels = [c for c in getattr(guild, 'text_channels', []) if c.permissions_for(getattr(guild, 'me', guild)).view_channel][:3]
        default_chans = [("general-chat", "3.2k"), ("media-share", "1.4k"), ("bot-commands", "780")]
        chan_data = []
        for i in range(3):
            if i < len(channels):
                name = channels[i].name[:14]
    ...
    @classmethod
    async def generate_user_stats_card(cls, member: Any, mod_counts: Dict[str, int]) -> io.BytesIO:
        """
        ...
        if avatar_url:
            try:
                av_img = await cls._fetch_image(avatar_url.url)
                if av_img:
        ...
        display_name = getattr(member, 'display_name', 'SyncInk User')
        ...
        subtitle = f"ID {member.id}  ·  /{guild_name.lower().replace(' ', '-')}"
        ...
        strip_data = [
            ...
            ("Tenure", f"{tenure_days}d", f"Joined {joined_dt.strftime('%b %Y')}", COLOR_ACCENT_ORANGE),
        ...
        top_role_name = top_role.name if top_role and top_role.name != "@everyone" else "Member"
